...
### 背景
- 微信开发者工具环境下偶发 503；根因是 Worker 在线拉取公钥不稳定 + 路径末尾斜杠未被拦截到加密分支。

### 变更
- Cloudflare Worker（`tools/cloudflare/worker.js`）
  - 内置后端 RSA 公钥（SPKI DER Base64），移除网络拉取依赖；在边缘执行 RSA-OAEP(SHA-256) 加密。
  - 仅对 `POST /w-game-service/api/v1/word/verify` 执行加密转发，容忍末尾斜杠；为排错保留 `X-Edge-Debug/X-Upstream-Status`。
- 客户端（`src/cocos/assets/scripts/services/NetworkService.ts`）
  - 超时从 2s → 5s，提高容错。
  - 清理成功路径冗余日志，仅保留失败必要日志。
  - 修复误输入字符导致的编译错误；新增 `declare const wx: any;` 以通过类型检查。

### 影响
- 明文→Worker 加密→后端解密的统一链路在微信开发者工具/浏览器/真机表现一致，稳定性提升；日志更精简，减少上下文占用。

### 后续（文档精简）
- CHANGELOG 体积偏大，后续将把 2025-10-25 之前的详细记录迁移到 `docs/archive/CHANGELOG-ARCHIVE.md`，主文件仅保留近期关键变更摘要。